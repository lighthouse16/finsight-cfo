...
**Important**: Data Room ingestion remains preview-only. Production analysis is not replaced, no files are permanently stored, no preview state is written to a database, and demo/provider data remains active where preview data is unavailable. OCR adapter stubs, PDF, and DOCX text extraction are implemented, but cloud OCR extraction requires provider credentials.
...
| ID | Task | Repo Status | Evidence |
|---|---|---|---|
| 2.1 | Golden Timing Index v1 | **Done** | [timing_signal_service.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/timing_signal_service.py) computes a context-only timing signal using HIBOR/HONIA rate trends, calendar red flags, and sector health context. Integrated with CME FedWatch Expectations adapter. [test_market_watch.py](file:///D:/projects/finsight-cfo/backend/tests/test_market_watch.py) validates safe wording, band edges, and calendar triggers. |
| 2.2 | Industry Health Context v1 | **Done** | [industry_health_service.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/industry_health_service.py) builds a context-only industry health signal. Integrated with IHS Sector Benchmarks adapter. [test_industry_health.py](file:///D:/projects/finsight-cfo/backend/tests/test_industry_health.py) validates sector contexts, default fallback, and safe wording. |
| 2.3 | Funding Channel Ranking v1 | **Done** | [funding_channel_ranking_service.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/funding_channel_ranking_service.py) ranks candidate funding channels based on workspace-derived company context, timing signal, and industry health. Integrated with `ProductCatalogService` to dynamically query and match catalog products based on use cases. Matches generic JSON catalog or BOCHK catalog provider when configured. |
| 2.4 | Cross-border Funding Context v1 | **Done** | [cross_border_funding_context_service.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/cross_border_funding_context_service.py) compares HKD/HIBOR vs RMB/LPR-style funding context. Integrated with ChinaData.live Macro Sector adapter. [test_cross_border_funding_context.py](file:///D:/projects/finsight-cfo/backend/tests/test_cross_border_funding_context.py) validates shape, bands, warnings, disclaimer, and forbidden terms. |
| 2.5 | Red Flags & Macro Risk Summary v1 | **Done** | [red_flags_macro_summary_service.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/red_flags_macro_summary_service.py) consolidates all existing Phase 2 signals. Headline, suggested review actions, and mitigants. Graceful degradation: if any underlying service fails, adds a warning and continues with partial data. [test_red_flags_macro_summary.py](file:///D:/projects/finsight-cfo/backend/tests/test_red_flags_macro_summary.py) validates. |
| 2.6 | Source Registry Hardening v1 | **Done** | Centralized source metadata registry at [source_registry.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/source_registry.py). Standardized registry entries mapping to the 6 core source modes (`live`, `provider_configured`, `provider_not_configured`, `workspace_derived`, `fixture`, `unavailable`). Frontend mirror at [sourceMeta.ts](file:///D:/projects/finsight-cfo/src/features/market-watch/utils/sourceMeta.ts) and tooltips. |
| 2.7 | Provider Integration Hardening v1 | **Done** | Core provider adapters implemented in [provider_adapters.py](file:///D:/projects/finsight-cfo/backend/app/services/market_watch/provider_adapters.py) (`CMEFedWatchAdapter`, `ChinaDataMacroSectorAdapter`, `IHSSectorBenchmarkAdapter`, `BOCHKProductCatalogAdapter`). They check for env-configured credentials and set appropriate status mode and warnings. |

**Phase 2 verdict**: 🟢 **7 Done.** Golden Timing Index, Industry Health Context, Funding Channel Ranking, Cross-border Funding Context, Red Flags & Macro Risk Summary, Source Registry Hardening, and Provider Integration Hardening are all implemented and fully integrated. All Phase 2 signals support live public feeds and explicitly disclose configured/not-configured paid credentials or fixture fallback warnings.
...
- Real text-layer PDF/DOCX parsing and OCR stub adapters are implemented, but real cloud OCR extraction requires API keys
